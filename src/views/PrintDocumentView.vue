<template>
  <div class="print-document">
    ...
    <br />
    <div>
      Общество с ограниченной ответственностью "{{ store.companyInfo.name }}", именуемое в дальнейшем «Исполнитель», в
      лице Генерального директора, действующего на основании Устава, с одной стороны и гражданка(-нин)
      {{ client.name }}, именуемый(-ая) в дальнейшем «Заказчик», вместе в дальнейшем именуемые Стороны, заключили
    ...
    <div class="print-document__block-text">
      <span>1</span>
      <p>Настоящее Приложение составлено и подписано Сторонами в соответствии и во исполнение п. 1.1 договора
        возмездного оказания услуг от {{ new Date().toLocaleDateString() }}.</p>
    </div>
    <div class="print-document__block-text">
      <span>2</span>
      <div>Исполнитель обязуется в соответствии с договором возмездного оказания услуг от  {{ new Date().toLocaleDateString() }}, оказать консультационные услуги Заказчику по следующим вопросам:
        <ul>
...
const order = JSON.parse(localStorage.getItem('orders')).find((order) => order.id === +route.params.id)
const client = JSON.parse(localStorage.getItem('clients')).find((client) => client.id === order.clientId)
const cost = order.serviceIds.map((id) => JSON.parse(localStorage.getItem('services')).find((service) => service.id === id).cost).reduce((a, b) => a + b, 0)
const services = order.serviceIds.map((id) => JSON.parse(localStorage.getItem('services')).find((service) => service.id === id).name).join(', ')
