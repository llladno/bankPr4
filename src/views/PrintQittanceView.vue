...
const containedData = {
  id: order.id,
  clientId: JSON.parse(localStorage.getItem('clients')).find((client) => client.id === order.clientId).name,
  employeeId: JSON.parse(localStorage.getItem('employees')).find((employee) => employee.id === order.employeeId).name,
  serviceIds: order.serviceIds.map((id) => JSON.parse(localStorage.getItem('services')).find((service) => service.id === id).name).join(', '),
  orderDate: order.orderDate,
  cost: order.serviceIds.map((id) => JSON.parse(localStorage.getItem('services')).find((service) => service.id === id).cost).reduce((a, b) => a + b, 0),
  status: order.status,
}
